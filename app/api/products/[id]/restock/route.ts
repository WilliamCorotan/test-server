...
export async function POST(
  ...
  try {
    ...
    const product = await db
      ...
      .from(products)
      .where(
        and(eq(products.id, parseInt(params.id)), eq(products.clerkId, userId))
      )
      .limit(1);
    ...
    const date = new Date();
    const dateOfRestock = `${date.getFullYear()}-${String(
      date.getMonth() + 1
    ...
    await db.transaction(async (tx) => {
      ...
      await tx
        ...
        .set({
          stock: newStock,
          expirationDate: expirationDate || currentProduct.expirationDate,
        })
      ...
      await tx.insert(restockHistory).values({
        ...
        newExpirationDate: expirationDate || currentProduct.expirationDate,
        dateOfRestock,
        notes,
        clerkId: userId,
      });
