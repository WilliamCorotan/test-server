...
export async function createRefund(data: RefundFormData, userId: string) {
  try {
    ...
    return await db.transaction(async (tx) => {
      const date = new Date();
      const dateOfRefund = `${date.getFullYear()}-${String(
        date.getMonth() + 1
      ...
      const refundId = parseInt(newRefund.lastInsertRowid?.toString() ?? "0");
...
export async function getRefundsByTransactionId(
  ...
  return db
    ...
    .from(refunds)
    .where(
      and(eq(refunds.transactionId, transactionId), eq(refunds.clerkId, userId))
    )
    .orderBy(refunds.dateOfRefund);
}
