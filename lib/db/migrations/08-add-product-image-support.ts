...
export async function addProductImageSupport() {
  try {
    // Check if the column already exists
    const imageUrlColumnExists = await db.get<{ count: number }>(sql`
            SELECT COUNT(*) as count
            FROM pragma_table_info('products')
            WHERE name = 'image_url';
        `);

    if (!imageUrlColumnExists || imageUrlColumnExists.count === 0) {
